...
| Goose and other stdio MCP clients | `npx -y @contracko/mcp` | launcher in `packages/mcp`; publishes to the public npm registry, not GitHub Packages. Proxies stdio to `https://app.contracko.com/mcp`. Not a second MCP server. First version is a local `npm publish`; later versions use npm trusted publishing (GitHub OIDC) from `release.yml`. |
...
- **Public npm for `@contracko/mcp`.** Goose and other stdio clients run `npx -y @contracko/mcp`. That package lives in `packages/mcp`. The first version is published locally (account 2FA). Later versions publish from the `v*` release workflow via npm trusted publishing (OIDC), not a long-lived `NPM_TOKEN`. GitHub Packages is the wrong registry for this: Goose cannot fetch it. The launcher is not a second MCP server.
